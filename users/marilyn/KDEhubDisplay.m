...
% Gaussian
grid_vals = zeros(ld*lr*lk,4);
names_grids = cell(ld*lr*lk,1);
count = 1;
...
%             plot(thr*ones(1,10),min(N_k):(max(N_k)-min(N_k))/9:max(N_k),'-k')
            ...
            names_grids{count} = ['dim= ' num2str(dim(i)) ', 1 to ' num2str(dr(j)) ',k = ' num2str(knn(k))];
            grid_vals(count,:) = 100*[tl, tr bl br]./size(q,1);
%             100*[tl tr]./(tl+tr)
...
%             plot(thr*ones(1,10),min(N_k):(max(N_k)-min(N_k))/9:max(N_k),'-k')
            ...
            bl = sum(nothubs.*notdense);
            br = sum(nothubs.*dense);
            names_grids2{count} = ['dim= ' num2str(dim(i)) ', 1 to ' num2str(dr(j)) ',k = ' num2str(knn(k))];
            grid_vals2(count,:) = 100*[tl, tr bl br]./size(q,1);
%             100*[tl tr]./(tl+tr)
%             pos = [(max(q)-min(q))/4 3*(max(N_k)-min(N_k))/4;3*(max(q)-min(q))/4 3*(max(N_k)-min(N_k))/4;(max(q)-min(q))/4 (max(N_k)-min(N_k))/4;3*(max(q)-min(q))/4 (max(N_k)-min(N_k))/4];
%             pos = [.105,210;.1305,210;.105,25;.1305,25];
%             for it=1:4
...
ave_top_ratio = mean([grid_vals2(:,2);grid_vals(:,2)]./sum([grid_vals2(:,1:2);grid_vals(:,1:2)],2))
ave_right_ratio = mean([grid_vals2(:,2);grid_vals(:,2)]./sum([grid_vals2(:,[2,4]);grid_vals(:,[2,4])],2))
